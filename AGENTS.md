...
- Default every new recipe and demo run to mock mode. Live provider spend requires an explicit user request.
...
- `packages/cli`: publishable `@zap-md/cli` package with the `zap` binary.
