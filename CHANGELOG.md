...
- Harness templates (Z10): `packages/templates/zap-heavy` named-snapshot base (Node 24, Docker, OpenViking/context7/open-connector MCP, API store, skills) plus 14 harness templates — named snapshots `zap-heavy-{hermes,openclaw,opencode}` and overlays `zap-heavy-{deepseek,grok,omg,pi,cursor,devin,kimi,agno,prime,headlong,frontier}` — each with `bake.sh`, `doctor.sh`, pinned `template.json`, systemd units, and AGENTS instructions; ≤ 6 named snapshots total.
- Harness runtime (Z10): pinned `HarnessManifest`/`HarnessService` modules for every harness under `packages/runtime/src/harness/`, shared run adapters (`http-runs`, `openai-compat`, `rpc-jsonl`, `cli-exec`, `ws-jsonrpc`) normalizing every stream to the redacted `RunEvent` union (goldens in `packages/runtime/tests/fixtures/harness-events/`), Hermes airv2 invariants (one user/one box, `noEnv`, only `api_server` inbound, private-hosted 8642/9119, `hermes-host.service` re-hosting after resume), managed-mode gateway wiring (`ZAP_API_URL/v1/runtimes/{id}/gateway`; provider keys never enter managed VMs), and Devin as a pull-only outbound worker.
- `zap harness` CLI (Z10): `ls`, `bake <template>` (plan-only by default; `--live` runs `infra/box/build-template.sh`), `doctor <id|template>`, and `run <template> --prompt` (plan-only event stream), with `--json` fixtures under `packages/cli/tests/fixtures/harness-*.json`; `infra/box/secret-sweep.sh` sweeps template dirs and named-snapshot boxes for provider/Box/Thirdweb/CDP/MPP/runtime-token patterns; docs under `docs/harnesses/` and `docs/templates/zap-heavy*.md`.
- Agents as code (Z12): `@wzrdtech/zap-agent` public API (`defineAgent`, `defineTool`, `defineConnection`, hooks, `useSecret`/`bearer`, plus `defineMcpServer`/`defineProject`/`defineRecipeTool`) with a synchronous render frame, I/O/async/type render guards, per-render capability rebuild, esbuild bundling into immutable sha-addressed deployments with value-free manifests, and build lint (`ZAP_BUILD_SECRET_LITERAL`, `ZAP_BUILD_ORIGIN_NOT_HTTPS`, `ZAP_BUILD_PROCESS_ENV`, `ZAP_BUILD_ASYNC_AGENT`, `ZAP_BUILD_UNDECLARED_SUBAGENT`, `ZAP_BUILD_UNDECLARED_MCP`).
- Agent host (Z12): in-VM `agents.host` under `packages/runtime/src/agentd/agents/` (deployments, alias history, durable sessions pinned to their `deploymentId`, serialized turns with `SESSION_BUSY`, render-before-every-model-step, plan-only `tool.planned`, subagent delegation, SSE event streaming) mounted at `/v1/agents`; runtime `SecretResolver` (env + control-plane) and connection fetch with manifest allowlisting (`CONNECTION_*`, `SECRET_SCOPE_DENIED`).
- Cloud (Z12): tenant-scoped `/v1/sessions` proxy mirroring metadata only, and the runtime-token `POST /v1/runtimes/:id/secrets/resolve` route with scope checks before value release.
- CLI (Z12): `zap agent new|ls|render|lint`, `zap deploy --agent` (`--watch` advances only `development`; `--alias production --sha` moves an existing deployment), `zap session --agent <id>[@alias] [--session] [--live] --json` emitting the JSONL event union with fail-closed `PAYER_MISSING`, `zap sessions ls`, and `zap secret set|list|remove|sync` with AES-256-GCM encryption at rest and last4-only listings.
- MCP + skills (Z12): `packages/mcp/src/tools/agents.js` (`zap_agent_ls`, `zap_agent_render`, `zap_deploy_agent`, `zap_session`, `zap_sessions_ls`, `zap_secret_list`; no secret-writing tool), the `skills/zap-agents` skill, the Studio bridge `zapAgentTool()` in `packages/agent/src/zap-bridge.ts`, and the dry-run eval `evals/agents-transcode.eval.ts`.
- MCP (Z8): auto-registered tool modules (`packages/mcp/src/tools/<domain>.js` exporting `toolNames` + `register`) covering compose/runtime/fs/exec/pay/memory/media/doctor plus the ten 0.3.1 recipe tools, §5.10 read-only/destructive annotations, payer-gated `live:true` returning structured `PAYER_MISSING`, and a Streamable HTTP transport (`zap mcp --http`) that binds `127.0.0.1` by default and requires `ZAP_MCP_TOKEN` (constant-time bearer check) for non-loopback binds.
- Skills (Z8): the `SKILL.md` contract in `packages/core/src/skill-manifest.ts` (frontmatter `name`/`description`/`version`, optional `metadata.zap.{weight,lanes,harnesses}`, VM store `/zap/skills/<name>/SKILL.md`), six new skills (`zap-runtime`, `zap-compose`, `zap-templates`, `zap-pay`, `zap-memory`, `zap-lanes`, each ≤ 2 KB), and regenerated skill manifests.
- API store (Z8): `apistore.context7` (hosted `https://mcp.context7.com/mcp`, `CONTEXT7_API_KEY` env reference), `apistore.open-connector` (self-hosted, loopback-only `http://127.0.0.1:3000/mcp`, pinned ref, per-box `OOMOL_CONNECT_*` env), `apistore.composio` (tenant-entity hosted MCP session), a fragment renderer for every harness `mcpConfig` format (yaml/json/json5/toml/cli), a 102-entry typed catalog rendered to `docs/catalog.md`, and `packages/templates/zap-heavy/bake.d/50-apistore.sh`.
- Agent plugin (Z8): `.claude-plugin/plugin.json` (MCP via `npx -y @wzrdtech/zap mcp`), `docs/agent-plugin.md` install snippets for six harness config formats, and `scripts/generate-llms-txt.mjs` regenerating `public/llms.txt` from the Appendix C template.
