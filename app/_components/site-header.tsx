...
import { WalletSignInButton } from "./wallet-sign-in-button";
  ...
  return (
    <header className="sticky top-0 z-50 border-white/10 border-b bg-[#07090d]/95 text-white shadow-lg backdrop-blur">
      <div className="mx-auto flex min-h-16 w-full max-w-7xl items-center gap-4 px-4 sm:px-6 lg:px-8">
        <Link className="flex min-h-11 items-center gap-2.5" href="/" prefetch={false}>
          <span className="flex size-9 overflow-hidden rounded-md border border-white/15 bg-black">
            <Image alt="Zap" className="h-full w-full object-cover" height={48} src="/zaplogo.png" width={48} />
          </span>
          <span className="font-semibold tracking-tight">Zap</span>
        </Link>
        <nav aria-label="Primary" className="flex min-w-0 flex-1 items-center gap-1 text-sm">
          <HeaderLink href="/gallery">Gallery</HeaderLink>
          <HeaderLink href="/docs">Docs</HeaderLink>
          <HeaderLink href="/studio">Studio</HeaderLink>
        </nav>
        <Suspense fallback={<WalletSignInButton />}>
