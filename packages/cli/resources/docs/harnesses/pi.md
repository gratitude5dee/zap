...
Stdio JSONL RPC (`rpc-jsonl`); no hosted ports. Each turn is one JSONL request/response over the CLI.

| field | value |
| --- | --- |
| run adapter | `rpc-jsonl` |
| min weight | `heavy` |
| template | zap-heavy-pi (overlay of zap-heavy) |
| ports | none |
| pins | `@earendil-works/pi-coding-agent@0.3.2` |
| units | — |
| state dirs | `~/.pi/agent` |
| MCP config | `~/.pi/agent/settings.json` (json) |
| LLM auth | `OPENAI_API_KEY` (byok), `ANTHROPIC_API_KEY` (byok) |
| managed gateway | `providers.zap.baseUrl` in `~/.pi/agent/settings.json` (openai-flavor proxy at `ZAP_API_URL/v1/runtimes/{id}/gateway`) |
