#!/usr/bin/env bash
# zap-heavy-devin doctor: base checks + harness checks.
set -euo pipefail
...
check "devin binary" command -v devin
check "devin-worker.service enabled" systemctl is-enabled devin-worker.service
check "outbound-only (no hosted port)" bash -c 'true'
