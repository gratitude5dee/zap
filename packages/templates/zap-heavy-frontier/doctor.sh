#!/usr/bin/env bash
# zap-heavy-frontier doctor: base checks + harness checks.
set -euo pipefail
...
check "frontier-agent" "${HOME}/.frontier/venv/bin/frontier-agent" --version
check "mcp.json" test -s "${HOME}/.frontier/mcp.json"
